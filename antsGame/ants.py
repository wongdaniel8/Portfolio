...
# Name:Daniel Riley Wong
# Email:wongdaniel8@berkeley.edu 
...
class Place:
        ...
        if insect.is_ant():
            ...
            if self.ant!=None:
                if self.ant.can_contain(insect): 
                    ...
                    ant_already_there=self.ant
                    self.ant=insect
                    self.ant.contain_ant(ant_already_there) 
                else: 
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            if removed_ant.container==True and removed_ant.ant!=None:
                self.add_insect(removed_ant.ant)  
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        new_place, index_of_new_place = self.place, 0
        while new_place != hive:
            if self.min_range<=index_of_new_place<=self.max_range and random_or_none(new_place.bees)!=None:
                return random_or_none(new_place.bees)
            new_place = new_place.entrance
            index_of_new_place=index_of_new_place+1
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if insect.watersafe==False:
            armor_amount=insect.armor
            insect.reduce_armor(armor_amount)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        Insect.reduce_armor(self,1) #can't call self.reduce_armor because that will cause infinite recursion 
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        QueenAnt.num_queens_made+=1
        if QueenAnt.num_queens_made>1:
            self.imposter=True  
        else:
            self.imposter=False 
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            #check front  
            cur_pos=self.place
            while cur_pos.exit!=None:
                if cur_pos.ant!=None and isinstance(cur_pos.ant,QueenAnt)==False and cur_pos.ant not in self.list_of_doubled:
                    cur_pos.ant.damage=cur_pos.ant.damage*2
                    self.list_of_doubled.append(cur_pos.ant)
                    if isinstance(cur_pos.ant,BodyguardAnt) and isinstance(cur_pos.ant.ant,QueenAnt)==False and cur_pos.ant.ant!=None and cur_pos.ant.ant not in self.list_of_doubled:
                        cur_pos.ant.ant.damage=cur_pos.ant.ant.damage*2
                        self.list_of_doubled.append(cur_pos.ant.ant)
                cur_pos=cur_pos.entrance 
            ...
            while cur_pos.exit!=None:
                if cur_pos.ant!=None and isinstance(cur_pos.ant,QueenAnt)==False and cur_pos.ant not in self.list_of_doubled:
                    cur_pos.ant.damage=cur_pos.ant.damage*2
                    self.list_of_doubled.append(cur_pos.ant)
                    if isinstance(cur_pos.ant,BodyguardAnt) and cur_pos.ant.ant!=None and isinstance(cur_pos.ant.ant,QueenAnt)==False and cur_pos.ant.ant not in self.list_of_doubled:
                        cur_pos.ant.ant.damage=cur_pos.ant.ant.damage*2
                        self.list_of_doubled.append(cur_pos.ant.ant)
