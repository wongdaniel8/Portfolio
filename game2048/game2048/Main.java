...
public class Main {
                ...
                if (r != 0 && board[r][c] != 0) {
                    if (tileInCol(board, r, c)) {
                        ...
                        if (board[rowClosestTile][c] == board[r][c]
                            && mergeBoard[tiltRow(side, rowClosestTile, c)]
                            [tiltCol(side, rowClosestTile, c)] == 0) {
                            _game.mergeTile(board[r][c], board[r][c] * 2,
                                tiltRow(side, r, c), tiltCol(side, r, c),
                                tiltRow(side, rowClosestTile, c),
                                tiltCol(side, rowClosestTile, c));
                            turnScore += board[r][c] * 2;
                            _board[tiltRow(side, rowClosestTile, c)]
                            [tiltCol(side, rowClosestTile, c)] =
                                _board[tiltRow(side, r, c)]
                                [tiltCol(side, r, c)] * 2;
                            mergeBoard[tiltRow(side, rowClosestTile, c)]
                            [tiltCol(side, rowClosestTile, c)] = 1;
                            _board[tiltRow(side, r, c)]
                            [tiltCol(side, r, c)] = 0;
                            board[rowClosestTile][c] = 2 * board[r][c];
                            board[r][c] = 0;
                        } else {
                            moveIt(side, board, r, c);
                        }
                    } else {
                        moveIt(side, board, r, c);
                    }
    ...
    /** Method to adjust the score, given the TURNSCORE. */
    public void adjustScore(int turnScore) {
        if (turnScore + _score > _maxScore && (_maxScore != 0)) {
    ...
    /** Method to move tile, if not mergeable, called within tiltBoard.
     *SIDE, BOARD, R, C, MERGEBOARD*/
    public void moveIt(Side side, int[][] board, int r, int c) {
        int validRow = furthestValidRow(board, r, c);
        _game.moveTile(board[r][c],
            tiltRow(side, r, c), tiltCol(side, r, c),
            tiltRow(side, validRow, c),
            tiltCol(side, validRow, c));
        if (validRow != r) {
            _board[tiltRow(side, validRow, c)]
                [tiltCol(side, validRow, c)] =
                board[r][c];
            _board[tiltRow(side, r, c)]
                [tiltCol(side, r, c)] = 0;
    ...
    /**Return the furthest valid row of a tile at row R and col C,
     *can be the same row, furthest valid row is always in the same column,
     *and with row<=current row, in BOARD.*/
     ...
     *the tile. Given BOARD, row R, and col C.*/
