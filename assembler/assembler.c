...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    int is_error = 0;
    char *token;
    char instruction_name[BUF_SIZE];
    char *args[MAX_ARGS];
    int i;
	...
	skip_comment(buf);
	printf("buf %s\n", buf);
	char* label_pos = strtok(buf, " ,.\f\n\r\t\v()");
	if (label_pos == NULL) {
	    line += 1;
	    continue;
	}
	printf("label_pos %s\n",  label_pos);
	int label = add_if_label(line, label_pos, byte_offset, symtbl);
	printf("label is %d\n", label);
	if (label == 0) {
	    strcpy(instruction_name, label_pos);
	    printf("11 instruct name %s\n", instruction_name);
	}
	if (label == -1) {
	    is_error = 1;
	    token = strtok(NULL, " ,.\f\n\r\t\v()");
	...
	*/
	...
	if (num_args > MAX_ARGS) {
	    ...
	    raise_extra_arg_error(line, args[num_args - 1]);
	    is_error = 1;
	}
	printf("reached b");
	if (is_error == 0) {
	    printf("NO ERROR!\n");
	    int write_pass = write_pass_one(output, instruction_name, args, i);
	    if (write_pass == 0) {
		printf("is error\n");
		is_error = 1;
	    } else {
		byte_offset += 4*write_pass;
	    }
	}
	line += 1;
    }
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    while (fgets(buf,BUF_SIZE, input)) {
        ...
        token = strtok(buf, " ,.\f\n\r\t\v()");
        if (token == NULL) line += 1;
        else strcpy(command_name, token);
	//printf("last char of instruct %c\n", command_name[strlen(command_name)-1]);
	...
	for (int i = 0 ; i < num_args; i++) {
        ...
        int translate = translate_inst(output, command_name, args, num_args, byte_offset, symtbl, reltbl);
        printf("translate %d\n", translate);
        byte_offset += 4;
        if (translate == -1) {
            raise_inst_error(line, command_name, args, num_args);
            ret = -1;
        }
	line += 1;
    }
