...
class SqueezeGame:
        ...
        def runGame(self, mvc, LR_TARGETL, LR_TARGETH, HR_TARGETL, HR_TARGETH, L_EFFORT_PROB, L_EFFORT_MAG, H_EFFORT_PROB, H_EFFORT_MAG, trialTime, hoverTime):
                """
                Runs a trial of the game, returns timer, average force exerted, and max force exerted
                """
                timeDur = time.time()
                total = 0.0								
                quited = False          				
...
##                sound.play(-1, 5000)
##                print("souynd length", sound.get_length())
                ...
                while not quited:
                       ...
                       #text for low lottery probability and magnitude
                        ...
                        while (samp > LR_TARGETL * float(mvc)) and (samp < LR_TARGETH * float(mvc))  and not quited:
                              ...
                              timeDur = time.time()
                              maxForceExerted = max(allSamples)
                              if timeDur - initTime > hoverTime: #return average squeeze as the midpoint between low range low and low range high
                                  return timer, ((LR_TARGETL * mvc) + (LR_TARGETH * mvc)) / float(2), maxForceExerted
                        ...
                        while (samp > HR_TARGETL * float(mvc)) and (samp < HR_TARGETH * float(mvc)) and not quited:
                              ...
                              timeDur = time.time()
                              maxForceExerted = max(allSamples)
                              if timeDur - initTime > hoverTime:
                                  return timer, ((HR_TARGETL * mvc) + (HR_TARGETH * mvc)) / float(2), maxForceExerted
                ...
                maxForceExerted = max(allSamples)
                return timer, summ / float(len(allSamples) - 200), maxForceExerted
...
def main(trial, mvc, MP):
        ...
        if trial != 0: 
            ...
            trialTime = paramsList[trial - 1][8]
            hoverTime = paramsList[trial - 1][9]
...
##        #==========================================================
        ...
        if trial > 0:
            ...
            t, avgSample, maxForceExerted = sg.runGame(mvc, LR_TARGETL, LR_TARGETH, HR_TARGETL, HR_TARGETH, L_EFFORT_PROB, L_EFFORT_MAG, H_EFFORT_PROB, H_EFFORT_MAG, trialTime, hoverTime)
...
##                forceOutputs.append((time.time() - initialTime, sg.mp.sample()[0] + .245, ""))
                ...
                returnVal = ("HR", mvc)
                if trial <= 20: 
                    returnVal = ("HR", max(mvc, maxForceExerted))
...
##                forceOutputs.append((time.time() - initialTime, sg.mp.sample()[0] + .245, ""))
                ...
                returnVal = ("LR", mvc)
                if trial <= 20: 
                    returnVal = ("LR", max(mvc, maxForceExerted))
...
##                forceOutputs.append((time.time() - initialTime, sg.mp.sample()[0] + .245, ""))
                ...
                returnVal = ("none", mvc)
                if trial <= 20:
                    returnVal = ("none", max(mvc, maxForceExerted))
            ...
            if trial != totalTrials - 1:
                ...
                if squeezed == False:
                    ...
                    if trial <= 20:
                        returnVal = ("failed", max(mvc, maxForceExerted))
                    disp.fill((0,0,0))
            ...
            if trial == totalTrials - 1 or (trial == totalTrials - 2 and squeezed == False):
                total = 0
                randomIndices = random.sample(range(0,len(winnings)), int(len(winnings) / float(5)))
                for i in randomIndices:
                    total += winnings[i]
                total = total / float(100)
                disp.fill((0,0,0))
                font1 = pygame.font.Font(pygame.font.get_default_font(), 33)
                textsurf = font1.render("You've completed the game! Your total winnings are:", False, (255,255,255))
                textsurf2 = font1.render("$"+str(total), False, (255,255,255))
                textsurf3 = font1.render("Thank you for participating. Please get the experimenter.", False, (255,255,255))
                disp.blit(textsurf,(320,200))
                disp.blit(textsurf2,(415,280))
                disp.blit(textsurf3,(315,360))
                pygame.display.flip()
                time.sleep(5)
...
while count != totalTrials: #6#count is number of trials + 1 for calibration
        ...
        if count == 0:
                ...
                #===========================================================               
                #modification for game to adjust mvc if use exerts a greater force than the mvc recorded during calibration 
                decisionRange, mvcForThisTrial = main(count, mvc, MP)
                if mvcForThisTrial > mvc and count <= 20:
                    mvc = mvcForThisTrial
                #===========================================================
