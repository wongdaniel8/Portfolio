...
def recordChoiceOutput(trialNumber):
    ...
    filename = "effort_MR1_" + str(subjectID) + ".txt"
...
def showStart(disp):
    example = pygame.image.load("start.png")
    disp.blit(example, (250,100))
    pygame.display.flip()
    while True:
        for event in pygame.event.get():                               
            if event.type == pygame.KEYUP and event.key == 53: #keycode for '5'
                return 
