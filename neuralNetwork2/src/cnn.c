...
static vol_t* copy_vol(vol_t* dest, vol_t* src) {
  int stride = dest->sx;
  int stride2 = dest->sy;
  int stride3 = dest->depth;
  double* target = dest->w;
  double* target2 = src->w;
  int d = src->depth;
  int sx = src->sx;
  for (int x = 0; x < stride; x++)
    for (int y = 0; y < stride2; y++)
      for (int z = 0; z < stride3; z++)
        target[((stride * y) + x) * stride3 + z] = target2[((sx * y) + x) * d + z];
}
/*
...
conv_layer_t* make_conv_layer(int in_sx, int in_sy, int in_depth, int sx, int filters, int stride, int pad) {
  ...
  int num2 = l->sy;
  int num3 = l->in_depth;
  vol_t** target = l->filters;
  target[0] = make_vol(num, num2, num3, 0.0); 
  target[1] = make_vol(num, num2, num3, 0.0); 
  target[2] = make_vol(num, num2, num3, 0.0); 
  target[3] = make_vol(num, num2, num3, 0.0); 
  target[4] = make_vol(num, num2, num3, 0.0);
  target[5] = make_vol(num, num2, num3, 0.0); 
  target[6] = make_vol(num, num2, num3, 0.0); 
  target[7] = make_vol(num, num2, num3, 0.0);
  target[8] = make_vol(num, num2, num3, 0.0); 
  target[9] = make_vol(num, num2, num3, 0.0); 
  target[10] = make_vol(num, num2, num3, 0.0); 
  target[11] = make_vol(num, num2, num3, 0.0); 
  target[12] = make_vol(num, num2, num3, 0.0); 
  target[13] = make_vol(num, num2, num3, 0.0); 
  target[14] = make_vol(num, num2, num3, 0.0); 
  target[15] = make_vol(num, num2, num3, 0.0); 			
  if (filters == 20) { 
    target[16] = make_vol(num, num2, num3, 0.0); 
    target[17] = make_vol(num, num2, num3, 0.0); 
    target[18] = make_vol(num, num2, num3, 0.0); 
    target[19] = make_vol(num, num2, num3, 0.0); 
  } 
  l->bias = 0.0;
...
void conv_forward(conv_layer_t* l, vol_t** in, vol_t** out, int start, int end) {
    ...
    int x = -l->pad;
    int orig = -l->pad;
    int stride = l->out_depth;
    int stride2 = l->out_sy;
    int stride3 = l->out_sx;
	...
	double* shot = V->w;
	    ...
	    double* bull = f->w;
	    if (num == 3) {
     	        for(int ay = 0; ay < stride2; y += xy_stride, ay+=1) {
                    x = orig;
                    for(int ax=0; ax < stride3; x += xy_stride, ax+=1) {
                      double a = 0.0;
                      for(int fx = 0; fx < stride4; fx+=1) {
                        int ox = x + fx;
			if (ox >=0 && ox < V_sx) {
                            for(int fy = 0; fy < stride5; fy+=1) {
                              int oy = y + fy;
                              if(oy >= 0 && oy < V_sy) {
	                        a += bull[((stride4 * fy)+fx)*num] * shot[((V_sx * oy)+ox)*num2];
                                a += bull[((stride4 * fy)+fx)*num+1] * shot[((V_sx * oy)+ox)*num2 + 1];
                                a += bull[((stride4 * fy)+fx)*num+2] * shot[((V_sx * oy)+ox)*num2 + 2];
                              }
                            }
	                  }
                      }
                      a += l->biases->w[d];
                      kill[(((sick * ay) + ax)*death + d)] = a;
                    }
              }
            } else if (num == 16) {
              for(int ay = 0; ay < l->out_sy; y += xy_stride, ay+=1) {
                x = orig;
                for(int ax=0; ax < l->out_sx; x += xy_stride, ax+=1) {
                  __m256d sseMul = _mm256_setzero_pd();
                  double a = 0.0;
                  for(int fx = 0; fx < f->sx; fx+=1) {
                    int ox = x + fx;
                    if (ox >=0 && ox < V_sx) {
                      for(int fy = 0; fy < f->sy; fy+=1) {
                        int oy = y + fy;
                        if(oy >= 0 && oy < V_sy) {
                          __m256d sseMul = _mm256_setzero_pd();
                          __m256d sseSum = _mm256_setzero_pd();
                          __m256d temp = _mm256_loadu_pd((bull)+(((stride4 * fy)+fx)*num));
                          __m256d temp2 = _mm256_loadu_pd((shot)+(((V_sx * oy)+ox)*num2));
                          sseMul = _mm256_mul_pd (temp, temp2);
                          sseSum = _mm256_add_pd (sseSum, sseMul);
                          temp = _mm256_loadu_pd((bull)+(((stride4 * fy)+fx)*num+4));
                          temp2 = _mm256_loadu_pd((shot)+(((V_sx * oy)+ox)*num2+4));
                          sseMul = _mm256_mul_pd (temp, temp2);
                          sseSum = _mm256_add_pd (sseSum, sseMul);
                          temp = _mm256_loadu_pd((bull)+(((stride4 * fy)+fx)*num+8));
                          temp2 = _mm256_loadu_pd((shot)+(((V_sx * oy)+ox)*num2+8));
                          sseMul = _mm256_mul_pd (temp, temp2);
                          sseSum = _mm256_add_pd (sseSum, sseMul);
                          temp = _mm256_loadu_pd((bull)+(((stride4 * fy)+fx)*num+12));
                          temp2 = _mm256_loadu_pd((shot)+(((V_sx * oy)+ox)*num2+12));
                          sseMul = _mm256_mul_pd (temp, temp2);
                          sseSum = _mm256_add_pd (sseSum, sseMul);
                          double sum[4];
                          _mm256_storeu_pd(sum, sseSum);
                          a += sum[0];
                          a += sum[1];
                          a += sum[2];
                          a += sum[3];
                    ...
                    if (ox >=0 && ox < V_sx) {
                      for(int fy = 0; fy < f->sy; fy+=1) {
                        ...
                        if(oy >= 0 && oy < V_sy) {
                          ...
                          sseMul = _mm256_mul_pd (temp, temp2);
                          sseSum = _mm256_add_pd (sseSum, sseMul);
                          temp = _mm256_loadu_pd((bull)+(((stride4 * fy)+fx)*num+16));
                          temp2 = _mm256_loadu_pd((shot)+(((V_sx * oy)+ox)*num2+16));
                          sseMul = _mm256_mul_pd (temp, temp2);
                          sseSum = _mm256_add_pd (sseSum, sseMul);
                          double sum[4];
                          _mm256_storeu_pd(sum, sseSum);
...
static inline void set_vol_mini(vol_t* v, int d, double val) {
  v->w[d] = val;
}
...
void net_forward(network_t* net, batch_t* v, int start, int end) {
	double init0 = timestamp_us();
  conv_forward(net->l0, v[0], v[1], start, end);
  	lay0 += timestamp_us() - init0;
	double init1 = timestamp_us();
  relu_forward(net->l1, v[1], v[2], start, end);
  	lay1 += timestamp_us() - init1;
	double init2 = timestamp_us();
  pool_forward(net->l2, v[2], v[3], start, end);
  	lay2 += timestamp_us() - init2;
  	double init3 = timestamp_us();
  conv_forward(net->l3, v[3], v[4], start, end);
  	lay3 += timestamp_us() - init3;
        double init4 = timestamp_us();
  relu_forward(net->l4, v[4], v[5], start, end);
        lay4 += timestamp_us() - init4;
	double init5 = timestamp_us();
  pool_forward(net->l5, v[5], v[6], start, end);
        lay5 += timestamp_us() - init5;
	double init6 = timestamp_us();
  conv_forward(net->l6, v[6], v[7], start, end);
        lay6 += timestamp_us() - init6;
	double init7 = timestamp_us();
  relu_forward(net->l7, v[7], v[8], start, end);
        lay7 += timestamp_us() - init7;
	double init8 = timestamp_us();
  pool_forward(net->l8, v[8], v[9], start, end);
        lay8 += timestamp_us() - init8;
        double init9 = timestamp_us();
  fc_forward(net->l9, v[9], v[10], start, end);
        lay9 += timestamp_us() - init9;
        double init10 = timestamp_us();
  softmax_forward(net->l10, v[10], v[11], start, end);
        lay10 += timestamp_us() - init10;
}
...
void net_classify_cats(network_t* net, vol_t** input, double* output, int n) {
  ...
  double init = timestamp_us();
  __m256d sseTot = _mm256_setzero_pd();
  #pragma omp parallel
  {
  batch_t* batch = make_batch(net, 1);
  #pragma omp for     
  for (int i = 0; i < n; i+=1) {
    copy_vol(batch[0][0], input[i]); //change one of indices 
  ...
  double convTime = lay0 + lay3 + lay6; 
  double reluTime = lay1 + lay4 + lay7;
  double poolTime = lay2 + lay5 + lay8;
  double fcTime = lay9;
  double softTime = lay10;
  double total = convTime + reluTime + poolTime + fcTime + softTime;

        printf("%s %f %f\n", "layer0:", lay0/1000, lay0/total*100);
        printf("%s %f %f\n", "layer1:", lay1/1000, lay1/total*100);
	printf("%s %f %f\n", "layer2:", lay2/1000, lay2/total*100);
	printf("%s %f %f\n", "layer3:", lay3/1000, lay3/total*100);
	printf("%s %f %f\n", "layer4:", lay4/1000, lay4/total*100);
	printf("%s %f %f\n", "layer5:", lay5/1000, lay5/total*100);
	printf("%s %f %f\n", "layer6:", lay6/1000, lay6/total*100);
	printf("%s %f %f\n", "layer7:", lay7/1000, lay7/total*100);
	printf("%s %f %f\n", "layer8:", lay8/1000, lay8/total*100);
	printf("%s %f %f\n", "layer9:", lay9/1000, lay9/total*100);
	printf("%s %f %f\n", "layer10:",lay10/1000, lay10/total*100);
 	printf("%s %f %f\n", "convTime:", convTime, convTime/total);
	printf("%s %f %f\n", "reluTime:", reluTime, reluTime/total);
        printf("%s %f %f\n", "poolTime:", poolTime, poolTime/total);
        printf("%s %f %f\n", "fcTime:", fcTime, fcTime/total);
        printf("%s %f %f\n", "softTime:", softTime, softTime/total);
}
