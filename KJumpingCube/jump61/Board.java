...
abstract class Board extends Observable {
    ...
    boolean isLegal(Side player, int n) {
        Square sq = get(n);
        if ((sq.getSide() != player) && (sq.getSide() != Side.WHITE)) {
            return false;
        }
        return true;
    }
    ...
    public String toString() {
        ...
        for (int i = 1; i < size() + 1; i++) {
            ...
            for (int j = 1; j < size() + 1; j++) {
                ...
                Side faction = get(i, j).getSide();
                if (j == size()) {
                    if (faction == Side.BLUE) {
                        answer += Integer.toString(spots) + "b\n";
                    } else if (faction == Side.RED) {
                        answer += Integer.toString(spots) + "r\n";
                    } else {
                        answer += Integer.toString(spots) + "-\n";
                    }
                } else {
                    if (faction == Side.BLUE) {
                        answer += Integer.toString(spots) + "b" + " ";
