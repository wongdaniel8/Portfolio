...
class Game extends Observable {
    ...
    void makeMove(int r, int c) {
        ...
        if (_verbose) {
            ...
            System.out.println("Red: " + _board.numOfSide(RED));
            System.out.println("Blue: " + _board.numOfSide(BLUE));
        }
    }
    ...
    private void setSpots(int r, int c, int spots, String color) {
        ...
        if (spots > _board.neighbors(r, c)) {
            reportError("spots must be less "
    ...
    private void restartGame() {
        ...
        if ((_players[0] instanceof HumanPlayer)
            ...
            while (_playing) {
                if (_board.whoseMove() == RED) {
                    _inp.nextLine();
                    if (promptForNext()) {
                        readAndExecuteMove(); checkForWin();
                    } else {
                        _exit = 0; _playing = false;
                    }
                    if (!_playing) {
                        break;
                    }
                ...
                if (_board.whoseMove() == BLUE) {
                    _players[1].makeMove(); checkForWin();
                }
            }
        } else if ((_players[1] instanceof HumanPlayer)
            && (_players[0] instanceof AI)) {
            while (_playing) {
                if (_board.whoseMove() == RED) {
                    _players[0].makeMove(); checkForWin();
                }
                if (!_playing) {
                    break;
                }
        ...
        } else if ((_players[0] instanceof AI) && (_players[1] instanceof AI)) {
            while (_playing) {
                if (_board.whoseMove() == RED) {
                    _players[0].makeMove(); checkForWin();
                }
        ...
        } else if ((_players[0] instanceof HumanPlayer)
    ...
    private void executeCommand(String cmnd) {
        ...
        case "quit":
            ...
            break;
        case "seed":
            System.out.println("Seed functionality invalid, AI does not have"
   ...
   /** Used to return a move entered from the console.  Allocated
    ...
    /** Indicates whether or not to print out the board and counts of each
     * side after a move. */
