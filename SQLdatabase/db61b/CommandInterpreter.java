...
class CommandInterpreter {
    ...
    Table selectClause() {
        ...
        if (_input.nextIs(";")) {
            if (secondTable == null) {
                ...
                return firstTable.select(secondTable, selectedCols, null);
            }
        }
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        ...
        Table secondTable = tables[1]; String relation = " ";
        String literal; boolean isLiteral = true; Condition cond;
        if (_input.nextIf("where")) {
            ...
            if (isLiteral) {
                ...
                cond = null;
                if ((firstTable.findColumn(literal) != -1)
                    || ((secondTable != null)
                    && (secondTable.findColumn(literal) != -1))) {
                    Column c2 = new Column(literal, firstTable, secondTable);
                    cond = new Condition(c, relation, c2);
                }
            }
            if (cond == null) {
                return null;
            ...
            while (_input.nextIf("and")) {
                Condition subsequent = getSubsequentCondition(condCols,
                    firstTable, secondTable);
                if (subsequent != null) {
                    conditions.add(subsequent);
                }
            }
            if (checkForColumnError(condCols, firstTable, secondTable)) {
                return null;
    ...
    /** Returns a condition, given CONDCOLS, FIRSTTABLE, SECONDTABLE. */
    Condition getSubsequentCondition(ArrayList<String> condCols,
        Table firstTable, Table secondTable) {
        boolean isLiteral = true;
        String relation = " ";
        String condCol, literal;
        Condition cond;
        condCol = _input.next();
        condCols.add(condCol);
        if (checkEarlyTermination()) {
