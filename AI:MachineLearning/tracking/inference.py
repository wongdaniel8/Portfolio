...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        """
        noisyDistance = observation
        emissionModel = busters.getObservationDistribution(noisyDistance) # dictionary that maps true position to P(Noisy Distance | True distance)
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        if all(val==self.getJailPosition() for val in self.particleList):
            beliefs[self.getJailPosition()] = 1
        else:
            for item in self.legalPositions:
                if item in self.particleList:
                    beliefs[item] = float(self.particleList.count(item)) / float(len(self.particleList))
                else:
                    beliefs[item] = 0
        return beliefs
...
class JointParticleFilter:
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particleList:
            ...
            for i in range(self.numGhosts): #looping through ghost index
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, prevGhostPositions), i, self.ghostAgents[i])
                newParticle[i] = util.sample(newPosDist)  #position of ghost in that particle
    ...
    def getBeliefDistribution(self):
        beliefs = util.Counter()
        for item in set(self.particleList):
            beliefs[item] = float(self.particleList.count(item)) / float(len(self.particleList))
        #print(beliefs)
        beliefs.normalize()
        return beliefs
