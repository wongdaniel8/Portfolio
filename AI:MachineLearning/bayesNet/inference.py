...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for variable in eliminationOrder:
            ...
            currentFactorsList.append(joinedFactor)
            newCurrentFactorsList = []
            for factor in currentFactorsList:
                if variable not in factor.unconditionedVariables():
                    newCurrentFactorsList.append(factor)
                elif len(factor.unconditionedVariables()) > 1:
                    newFactor = eliminate(factor, variable)
                    newCurrentFactorsList.append(newFactor)
            currentFactorsList = newCurrentFactorsList
        joined = joinFactors(currentFactorsList)
        returnvalue = normalize(joined)
        return returnvalue
