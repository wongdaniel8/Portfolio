...
(define (list-partitions total max-pieces max-value)
  ; *** YOUR CODE HERE ***
    ...
    (define (partition_maker total1 list1 max_pieces max_value)
      (cond ((= total1 0) (list list1))
            ((= max_value 0) nil)
            ((< total1 0) nil)
            ((= max_pieces 0) nil)
            
            (else (append (partition_maker (- total1 max_value) (cons max_value list1) (- max_pieces 1) max_value); (- max_value 1))
                          (partition_maker total1 list1 max_pieces (- max_value 1)))
            )
      )
    )
      (partition_maker total nil max-pieces max-value)
...
(define (tree-sums tree)
  ...
  (define (flatten lst1)
    (cond ((null? lst1) '() )
          ((list? lst1) (append (flatten (car lst1)) (flatten (cdr lst1))))
          (else (list lst1))    
    )
  )
